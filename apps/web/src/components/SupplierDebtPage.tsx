...
export const SupplierDebtPage: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto py-2 pb-12">
      ...
      <div className="w-full bg-surface-container-lowest p-5 rounded-2xl border border-outline-variant shadow-sm flex flex-col gap-4">
        <div className="flex justify-between items-center border-b border-surface-variant pb-3">
          ...
          <button
            ...
            className="flex items-center gap-1 bg-primary hover:bg-primary-container text-white px-3.5 py-1.5 rounded-xl text-xs font-semibold shadow-xs transition-transform active:scale-95"
          >
          ...
          <table className="w-full text-left text-xs border-collapse table-auto">
            <thead>
              <tr className="bg-slate-50/80 text-slate-500 font-semibold text-xs uppercase tracking-wider border-b border-slate-200">
                <th className="py-3 px-3 align-middle w-[32%]">Ta'minotchi Nomi & Tel</th>
                <th className="py-3 px-3 text-right align-middle w-[20%]">Jami Qarz</th>
                <th className="py-3 px-3 align-middle w-[18%]">To'lov Muddati</th>
                <th className="py-3 px-3 align-middle w-[15%]">Holat</th>
                  ...
                  return (
                    <React.Fragment key={s.id}>
                      <tr className="hover:bg-slate-50/80 transition-colors group">
                        <td className="py-3 px-3 align-middle w-[32%]">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="bg-slate-100 hover:bg-emerald-100 text-slate-600 hover:text-emerald-700 p-1.5 px-2 rounded-lg border border-slate-200 transition-colors flex items-center gap-1 flex-shrink-0"
                            ...
                            >
                              ...
                              <span className="text-[11px] font-medium text-slate-500">
                                {isExpanded ? 'Yopish' : 'Tafsilotlar'}
                              </span>
                            </button>

                            <div className="min-w-0">
                              <span className="text-sm font-semibold text-slate-800 block group-hover:text-emerald-700 transition-colors truncate">
                                {s.name}
                              </span>
                              {s.phone && (
                                <a
                                  href={`tel:${s.phone.replace(/\s/g, '')}`}
                                  className="text-[11px] font-medium text-slate-500 hover:text-emerald-700 hover:underline flex items-center gap-1 mt-0.5 whitespace-nowrap"
                                >
                        ...
                        <td className="py-3 px-3 text-right align-middle text-sm font-semibold text-rose-600 whitespace-nowrap w-[20%]">
                          {s.currentBalance.toLocaleString('ru-RU')} so'm
                        </td>

                        <td className="py-3 px-3 align-middle text-xs font-medium text-slate-600 whitespace-nowrap w-[18%]">
                          {s.dueDate ? (
                            <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-800 text-[11px] font-semibold px-2 py-0.5 rounded-md border border-amber-200">
                              Eng yaqin: {s.dueDate}
                            </span>
                          ) : (
                            'Belgilanmagan'
                            ...
                            <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm flex flex-col gap-3">
                              <div className="flex justify-between items-center pb-2.5 border-b border-slate-200">
                                ...
                                <button
                                  type="button"
                                  onClick={() => withAuthGuard(() => handleOpenAddDebtModal(s))}
                                  className="text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 px-3 py-1 rounded-xl transition-colors flex items-center gap-1 shadow-2xs"
                                >
                              ...
                              {debtsList.length === 0 ? (
                                <div className="text-center py-4 text-xs font-medium text-slate-500 bg-slate-50 rounded-xl">
                                      ...
                                      {debtsList.map((d) => {
                                        ...
                                        return (
                                          <tr key={d.id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="py-2.5 px-3 text-right text-sm font-semibold text-rose-600 whitespace-nowrap">
      ...
      >
        <h3 className="font-headline font-semibold text-base text-slate-800 flex items-center gap-2 border-b border-surface-variant pb-3">
        ...
        <form onSubmit={handlePaymentSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-3.5 items-end w-full">
          <div className="md:col-span-4 flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-600">Ta'minotchi</label>
            <select
              value={selectedSupplierId}
              onChange={(e) => setSelectedSupplierId(e.target.value)}
              className="w-full px-3.5 py-2.5 rounded-xl border border-outline-variant bg-surface-container-low text-on-surface font-semibold text-xs focus:border-secondary outline-none"
            >
              {suppliers.map((s) => (
          ...
          <div className="md:col-span-4 flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-600">To'lov Summasi (so'm)</label>
            <input
              type="text"
              required
              value={paymentAmount}
          ...
          <div className="bg-surface-container-lowest border border-outline-variant rounded-3xl p-5 sm:p-6 shadow-2xl max-w-md w-full flex flex-col gap-4">
            <div className="flex justify-between items-center pb-2.5 border-b border-outline-variant/60">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-headline font-semibold text-base text-slate-800">
            ...
            <form onSubmit={handleAddDebtSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-outline-variant bg-surface-container-low text-on-surface font-medium text-xs focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none"
                />
              </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-outline-variant bg-surface-container-low text-on-surface font-semibold text-xs focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none"
                />
              </div>
              ...
              <div className="flex gap-2.5 pt-2">
                <button
                  ...
                  disabled={isSubmittingDebt}
                  onClick={() => setShowAddDebtModal(false)}
                  className="flex-1 py-2.5 rounded-xl text-xs font-semibold text-on-surface-variant bg-surface-container-high hover:bg-surface-variant transition-colors disabled:opacity-50"
                >
                  Bekor qilish
                </button>
                <button
                  type="submit"
                  disabled={isSubmittingDebt}
                  className="flex-1 py-2.5 rounded-xl text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 transition-colors shadow-sm flex items-center justify-center gap-1.5 disabled:opacity-50"
                >
                  {isSubmittingDebt ? (
                    <>
